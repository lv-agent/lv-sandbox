...
#[tokio::test]
async fn five_concurrent_jobs_all_complete() {
    ...
    for (i, result) in results.into_iter().enumerate() {
        let r = result.unwrap_or_else(|_| panic!("job {} should not error", i));
        assert!(
...
#[tokio::test]
async fn queueing_jobs_beyond_max_wait_not_rejected() {
    let (_tmp, runner) = create_test_runner().await;
...
#[tokio::test]
async fn actual_concurrency_does_not_exceed_max_concurrent() {
    let (_tmp, runner) = create_test_runner().await;
    let max = 3;
    let scheduler = std::sync::Arc::new(sandbox_server::scheduler::Scheduler::new(
        std::sync::Arc::new(runner),
    ...
    for r in results {
        assert!(r.is_ok(), "job should not error");
    }
}

#[tokio::test]
async fn concurrent_jobs_capture_correct_stdout() {
    let (_tmp, runner) = create_test_runner().await;
    let handles: Vec<_> = (0..5)
        .map(|i| {
            let runner = &runner;
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        assert!(
            ...
            "job {} stdout should contain UNIQUE_{}, actual: {}",
            i, i, stdout
        );
    }
}
...
#[tokio::test]
async fn rapidly_submit_20_jobs_stress_test() {
    let (_tmp, runner) = create_test_runner().await;
    let scheduler = std::sync::Arc::new(sandbox_server::scheduler::Scheduler::new(
        std::sync::Arc::new(runner),
        10,
    ...
    assert_eq!(ok_count, 20, "all 20 jobs should complete");
}
